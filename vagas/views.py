...
from empresa.models import Tecnologias
...
def nova_vaga(request):
    if request.method == "POST":
        ...
        return redirect(f'/home/empresa/{empresa}', {'techdomina':tecnologias_domina, 'techestudar':tecnologias_nao_domina})
...
def vaga(request, id):
    tecnologias_dominadas = Vagas.tecnologias_dominadas
    techestudar = Vagas.tecnologias_estudar
    vaga = get_object_or_404(Vagas, id=id)
    tarefas = Tarefa.objects.filter(vaga=vaga).filter(realizada=False)
    emails = Emails.objects.filter(vaga=vaga)
    return render(request, 'vaga.html', {'vaga':vaga, 'tarefas': tarefas, 'techdomina':tecnologias_dominadas, 'techestudar':techestudar})
